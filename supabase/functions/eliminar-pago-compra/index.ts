...
serve(async (req) => {
  ...
  try {
    ...
    if (!paymentId || !purchaseId) {
      throw new Error('Faltan paymentId y purchaseId para eliminar el pago.');
    }

    connection = await pool.connect();
    await connection.queryObject('BEGIN');

    // 1. Intentar eliminar el pago. No importa si no se encuentra (ya podría haber sido eliminado).
    await connection.queryObject(
      `DELETE FROM public."Pagos_Compra" WHERE "id" = $1;`,
      [paymentId]
    );
    
    // 2. SIEMPRE recalcular los totales y actualizar el estado de la compra para garantizar la coherencia.
    ...
    if (totalsResult.rows.length === 0) {
      // Este es un error genuino, la compra en sí no existe.
    ...
    return new Response(JSON.stringify({ success: true, message: 'Pago procesado y estado de la compra actualizado.' }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    });

  } catch (error) {
    console.error("Error in eliminar-pago-compra function:", error.message, error);
    if (connection) {
      try {
